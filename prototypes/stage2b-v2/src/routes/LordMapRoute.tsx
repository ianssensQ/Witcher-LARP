...
const lordMapFallbackNameBySocketId = Object.fromEntries(
  lordMapSockets.map((socket) => [socket.id, socket.name])
) as Record<string, string>;
const lordMapCyrillicNamePattern = /[А-ЯЁа-яё]/;
const lordMapFallbackLabelOffsetY = 72 / 1984 * 100;

const getLordMapSocketUiName = (socketId: string, name: string) => {
  const trimmedName = name.trim();
  if (trimmedName && lordMapCyrillicNamePattern.test(trimmedName)) {
    return trimmedName;
  }

  return lordMapFallbackNameBySocketId[socketId] ?? trimmedName ?? socketId;
};

const getLordMapSocketLabelStyle = (socket: LordMapSocket) => ({
  left: `${Number.isFinite(socket.labelX) ? socket.labelX : socket.x}%`,
  top: `${Number.isFinite(socket.labelY) ? socket.labelY : Math.max(2, socket.y - lordMapFallbackLabelOffsetY)}%`
});
...
const buildLordMapRuntimeSockets = (
  ...
  return mapNodes
    .map<LordMapSocket | null>((node) => {
      ...
      const labelAnchorX = Number(layoutNode.label_anchor?.x);
      const labelAnchorY = Number(layoutNode.label_anchor?.y);
      const costLabel = fallbackSocket?.route ?? "";
      const contestedByDomainId = territory?.contested_by_domain_id ?? null;
      const socketName = territory?.node_name || territory?.name || node.name || fallbackSocket?.name || nodeId;

      return {
        id: nodeId,
        name: getLordMapSocketUiName(nodeId, socketName),
        x,
        y,
        labelX: Number.isFinite(labelAnchorX) ? labelAnchorX / viewBox.width * 100 : undefined,
        labelY: Number.isFinite(labelAnchorY) ? labelAnchorY / viewBox.height * 100 : undefined,
...
const matchesLordMapTerritory = (territoryId: unknown, preferredTerritoryId?: string) => {
  if (!preferredTerritoryId) return true;
  return String(territoryId ?? "") === preferredTerritoryId;
};
...
const getLordMapActiveBattleId = (
  state: LordMapBackendState | null | undefined,
  preferredTerritoryId?: string
) => {
  const battle = [...(state?.active_battles ?? []), ...(state?.battles ?? [])].find((item) =>
    isLordMapReadyBattle(item) && matchesLordMapTerritory(item.territory_id, preferredTerritoryId)
  );
  if (battle?.battle_id) {
  ...
  const alertBattle = (state?.battle_alerts ?? []).find((item) => {
    ...
    return (
      item.type === "active_battle" &&
      Boolean(battleId) &&
      matchesLordMapTerritory(territoryId, preferredTerritoryId) &&
      !lordMapFinalBattleStatuses.has(status)
    );
  });
  return alertBattle ? String(alertBattle.cta?.battle_id ?? alertBattle.battle_id) : null;
};

const getLordMapPendingBattleClaim = (
  state: LordMapBackendState | null,
  preferredTerritoryId?: string
): LordMapPendingBattleClaim | null => {
...
function LordMapScreen() {
  ...
  const openLordMapBattle = useCallback(async (source = "stage2b_map_claim", preferredTerritoryId = "") => {
    const initialActiveBattleId = preferredTerritoryId
      ? getLordMapActiveBattleId(backendState, preferredTerritoryId)
      : activeBattleId;
    const initialBattleClaim = preferredTerritoryId
      ? getLordMapPendingBattleClaim(backendState, preferredTerritoryId)
      : mapBattleClaim;
    if (initialActiveBattleId) {
      navigateToLordMapBattle(initialActiveBattleId, preferredTerritoryId || initialBattleClaim?.territoryId);
      return;
    }

    if (initialBattleClaim?.battleId) {
      navigateToLordMapBattle(initialBattleClaim.battleId, initialBattleClaim.territoryId);
      return;
    }

    const refreshedState = await fetchLordMapState({ silent: true });
    const refreshedBattleId = getLordMapActiveBattleId(refreshedState, preferredTerritoryId || undefined);
    const refreshedBattleClaim = getLordMapPendingBattleClaim(refreshedState, preferredTerritoryId || undefined);
    if (refreshedBattleId) {
      navigateToLordMapBattle(refreshedBattleId, preferredTerritoryId || refreshedBattleClaim?.territoryId);
      return;
    }

    const battleClaim = refreshedBattleClaim ?? initialBattleClaim;
  ...
  useEffect(() => {
    ...
    void openLordMapBattle("stage2b_map_arrival", armySocket.territoryId);
  }, [
    arrivalBattleOpenRequestId,
    armySocket.territoryId,
    isBackendPendingMove,
    isMoveSubmitting,
    mapBattleClaim?.claimId,
    mapBattleClaim?.territoryId,
  ...
  const mapClaimBannerText =
    getLordMapPendingBattleClaim(backendState, displaySocket.territoryId) && !isBackendPendingMove
  ...
  const mapBattleSocket =
    hasMapModel &&
    (getLordMapActiveBattleId(backendState, armySocket.territoryId) || getLordMapPendingBattleClaim(backendState, armySocket.territoryId)) &&
    !movementDraft &&
    !isBackendPendingMove &&
    !isMoveSubmitting &&
      ...
      <div className="lord-map-game-stage">
        ...
        <section className="lord-map-board" aria-label="Карта земель">
          ...
          >
            ...
            <div className="lord-map-label-layer" aria-hidden="true">
              {mapSockets.map((socket) => {
                const isArmySocket = socket.id === armySocketId && !movementDraft && !isBackendPendingMove;
                const isRouteStart = socket.id === routeStartSocketId && !isArmySocket;
                const isRouteStep = displayPathIds.includes(socket.id) || activeRoutePathIds.includes(socket.id);
                const isRouteStop = routeStopSocketId === socket.id && hasDisplayRoute;
                const isPreviewTarget = isMarchMode && displaySocket.id === socket.id && !isArmySocket;
                const isHovered = hoveredSocketId === socket.id;
                const isResidence = isLordMapResidenceSocket(socket);

                return (
                  <span
                    key={`${socket.id}-label`}
                    className={`lord-map-territory-label ${socket.tone}${isResidence ? " is-residence" : ""}${selectedSocket.id === socket.id ? " is-selected" : ""}${isArmySocket ? " is-army-node" : ""}${isRouteStart ? " is-route-start" : ""}${isRouteStep ? " is-route-step" : ""}${isRouteStop ? " is-route-stop" : ""}${isPreviewTarget ? " is-preview-target" : ""}${isHovered ? " is-hovered" : ""}`}
                    style={getLordMapSocketLabelStyle(socket)}
                  >
                    {socket.name}
                  </span>
                );
            ...
            {mapBattleSocket && (
              <button
                ...
                onClick={() => {
                  void openLordMapBattle("stage2b_map_cta", mapBattleSocket.territoryId);
                }}
              >
                <Swords size={15} />
                Бой
