...
Проект описывает базовую структуру игры: карту владений, роли лордов, ведьмаков
и чародеек, PvE-проверки, эскроу-заказы, рынок, PvP-вызовы и модель боя лордов.
...
`.venv` по `pyproject.toml` и `uv.lock`.
...
Если IP мастерского ноутбука изменился, замените только `192.168.0.150` на
