...
- deterministic lord battle 5x6: `attack`, `defense`, `hp`, `initiative`, `move_range`, `attack_range`, `tier`, `unit_class`, damage `count_alive * max(1, effective_attack - effective_defense + modifiers)` with class bonuses, 60s turn timer, auto-resolve;
...
Актуальный рабочий запуск админки и игры лордов называется
продакшен-сервером: это один локальный FastAPI/SQLite процесс на
...
- атаку;
- damage `count_alive * max(1, effective_attack - effective_defense + modifiers)`, class bonuses and heavy_siege hero/building multiplier;
- ортогональное движение, `attack_range`, обычный line of sight и навесная атака ranged/heavy_siege через юнитов на пути;
...
- `launch-risk: firewall` - Windows firewall для порта `8002` должен быть
