...
7. **Unique Quest Production** (`TASK-032`) - production-контур контента: 72 QR/PvE-квеста, ручная полировка, full content pack, QR print/manual checklist и content smoke.
...
- Контент готовится в CSV, импортируется в SQLite и экспортируется в mobile snapshot; full pack V1 содержит 72 QR/PvE, 24 always-available/repeatable сцены и 48 unique story quests.
...
- QR/manual ID является слотом общей доски заказов: он не указывает физический prop/локацию, а спорные запуски уходят в master review.
- PvE checks используют серию проверок: каждая отдельная проверка = один app-generated `single_d20`, все преимущества/помехи учитываются как логируемые modifiers, базовая победа = 2 успеха из 3.
...
- offline PvE с app-generated d20-серией, выбором подхода, немедленным результатом и полным логом бросков/modifiers;
- предупреждение о честном использовании QR/manual ID с доски заказов и локальный лог подтверждения игрока;
...
Перед runtime CSV используется authoring-only матрица контента. Она нужна мастерам и контент-мейкеру, не обязана попадать в приложение напрямую и проверяет разнообразие 72 QR/PvE-квестов.

Минимальные поля матрицы: `quest_id`, акт, технический QR-slot, тир, тип сцены, opaque `manual_id`, `board_description`, `scan_reveal`, `choice_options_json`, hidden `choice_morality_json` с `good_evil_delta`, `encounter_steps_json`, `victory_rule`, видимый hook, скрытая правда, основной/вторичный стат, `combat_profile`, `reward_budget`, `check_policy`, `modifier_sources`, `rarity`, `power_budget`, `act_cap`, `visibility`, `counterplay`, `qr_mode`, `honesty_policy`, `act_unlock_policy`, `reward_approval_policy`, `reputation_flags`, `npc_flags`, `personal_goal_hooks`, `goal_flags`, `final_hook`, `final_summary_category`, `pvp_throttle_tag`, `ops_checklist_tag`, `player_facing_handout_tag`, `role_load_tag`, `sync_outcome`, `failure_text`, `success_text`, тестовый статус.
...
- `map_edges.csv` - связи узлов, movement point cost, terrain tags, travel/safety notes, route flavor и ограничения маршрута; route validation не должна проводить лошадь сквозь нейтральную, чужую или contested территорию без остановки.
...
- `territory_bonuses.csv` - индивидуальный стратегический бафф каждой игровой территории: доход, влияние, MP tempo, recruit unlock, raid token cap или raid defense; рантайм использует эти строки для tick-экономики, MP, найма и рейдовой защиты, а `bonus_type` остается грубой категорией для UI/дерева зданий. Территории не дают mana bonus, потому что мана не используется в текущей игре.
- `territory_forts.csv` - тематический форт каждой игровой территории, включая резиденции: `fort_id`, `territory_id`, `theme`, `visual_tag`, `art_prompt`, `garrison_capacity`, optional `defense_bonus`; V1 capacity считает слоты пачек и лежит в диапазоне `5..8`, а не ограничивает количество солдат внутри стека; каждая playable territory получает одну оригинальную картинку/карточку форта.
...
- `raid_rules.csv` - raid subtree, token-only costs, targets, defense/magic checks, debuff duration, optional loot effects `gold/cards/influence`.
- `mobs.csv` - мобы, HP, урон, награды, QR-ID, сценарий.
- `pve_scenarios.csv` - шаги PvE, проверки, тексты, переходы.
- `qr_objects.csv` - opaque QR/manual ID, тип, `qr_mode` (`unique_object`, `repeatable_scene`, `always_available_scene`), act availability, linked scenario/story slot, failure cooldown, manual-entry rate-limit policy и role load tags.
...
Телефон проводит PvE офлайн как один QR = один самостоятельный квест с доски заказов. Сцена поддерживает описание заказа, reveal после скана, выбор подхода, 3 шага боя/загадки/моральной проверки, награду, 30-минутный cooldown на конкретный QR после провала и полный roll/event log. Проверки используют серию `single_d20`: каждый шаг = один app-generated d20 + стат + бонусы предметов/зелий/артефактов/магии + логируемые modifiers преимущества/помехи; базовая победа = 2 успеха из 3. PvE combat v1 хранит `player_scene_hp`, `scene_hp`, `combat_dc`, `scene_damage`, `round_limit`, `timeout_outcome`, `base_damage` и не создает постоянного здоровья персонажа. Владение территорией не блокирует прохождение ведьмачьих/чародейских QR-квестов, а QR не привязан к физической локации. Будущие акты открываются только через server sync или master `act_unlock_code`, который валидируется при sync; code раскрывается после физического объявления акта. Сервер при sync проверяет QR, opaque/manual ID policy, режим потребления, act availability, unlock source, player cooldown, story slot state и reward approval policy. Cascade-prone rewards получают `pending_master_approval` и asset lock до решения мастера; `auto_approve_safe` награды могут применяться автоматически. Level-up grants +1 stat, max stat 7.

Проверяется после mobile shell и snapshot: QR/story scene -> act unlock -> branch choice -> 3 checks/short combat -> app restart -> sync -> visible master event -> reward approval/cooldown 30 min/story outcome.
...
Владение землей требует гарнизон в тематическом форте. После победы атакующий должен оставить минимум одну выжившую пачку в fort garrison; без гарнизона доход и основной бонус не активны. Owner может перебрасывать текущие army unit cards между активной армией и фортом, если активная армия находится на этой территории; transfer не тратит MP, но логируется, проверяет ownership, non-contested state, active battle lock, active army stack-slot capacity, fort `garrison_capacity` и правило minimum garrison. Чужие гарнизоны скрыты от других лордов, но owner и primary bonus type видны. Если hourly income/influence tick попадает на ongoing claim, сервер создает `pending_tick_reward` и применяет его победителю боя ровно один раз без сдвига расписания.

Найм в лордском UI показывается как накопление войск в выбранной локации: слот юнита отображает `+X/час` и текущий накопленный запас в скобках, а покупка через модальное окно со слайдером отправляет войска в гарнизон выбранной территории. Runtime cap для `current_stock` считается от темпа найма: без Склада максимум равен 2 тикам текущего `rate_per_hour`, со Складом - 4 тикам. Backend может сохранять compatibility с recruit offers, но player-facing интерфейс для замка/территорий не должен выглядеть как служебный список offers. Building tree покупается за gold по prerequisites без act cap. Резиденция имеет полный default catalog v1 из 4 веток: военная ветка (Учебный двор, Казармы, Стрельбище, Конюшни, Осадный двор, Военная академия), казна (Рынок, Налоговая палата, Склад, Банк, Казначейский зал), совет (Доска объявлений, Посольский зал, Картографическая, Рейдовая ставка, Военный совет), башня мага (Кабинет мага, Алхимическая лаборатория, Комната видений, Обереги, Ритуальная палата). Захваченные территории используют тот же building-tree UX с меньшим локальным деревом, которое позднее расширяется контентом. Anti-snowball rule режет income на 30% или 50%, если сила армии сильно или огромно выше средней. Diplomacy signals показывают мастеру/лордам поводы для союзов, заговоров и коалиций против лидера. Raid engine отделен от battle engine: raid token -> target validation -> defense/magic check -> timed debuff and optional gold/cards/influence loot. Orders capped at 2 public + 1 addressed active orders per lord, and lord progression remains playable without witcher availability.
...
Лордские бои синхронные в веб-панелях. Сервер хранит deterministic поле 5x6, стартовые линии, отдельные клетки лордов, "руку" deployment из доступных army unit cards, карты-отряды как стеки юнитов, `attack`, `defense`, `hp`, `initiative`, `move_range`, `attack_range`, `tier`, `unit_class`, инициативу по раундам, одну ответку за раунд, 60s turn timer, HP лорда с min/max cap от размера выставленной армии, сдачу в свой ход, сжигание уничтоженных army unit cards и итог. Damage: `count_alive * max(1, effective_attack - effective_defense + modifiers)`, где class bonuses усиливают дорогие классы, а heavy_siege имеет усиленный `hero_damage`. V1 runtime также хранит `unit_power`, `deployed_army_power`, `domain_army_power`, `count_alive`, `wounds_on_front_unit`, deployment cap, hero targeting and auto-resolve inputs. V1 не вводит отдельную обязательную матрицу "контр-модификатор x дистанция x местность"; дальность и line of sight ограничивают легальность атаки, а modifiers остаются плоскими статусами. Initiative tiebreaker: `initiative desc`, `tier desc`, deterministic battle seed. Движение ортогональное; ranged/heavy_siege используют навесную атаку по `attack_range` и не блокируются юнитами на пути. Timeout = auto-defend/skip; repeated timeout = auto-resolve/master takeover. Этот engine используется для нейтральной обороны, гарнизонов и столкновения армий; рейды не запускают 5x6 бой. V1 поддерживает классы `infantry`, `guard`, `ranged`, `cavalry`, `heavy_siege`, `specialist`; маги и монстры остаются нейтралами, редкими спецэффектами, NPC/артефактами или влиянием чародеек, а не массовыми лордскими юнитами.
...
- Unit tests: PvE, offline act unlock, reward approval locks, +1 stat level-up, personal_goals/goal_flags visibility, full Gwent deck/round/tie/effects, PvP challenge tokens/window/refusal/tie/throttle, trade_transfers lock/accept/decline, deterministic lord battle 60s timeout/auto-resolve, fort garrison transfer/capacity/minimum rules, escrow, order object conflict and order cap, cooldown 30 min, QR consumption, auto timers, anti-snowball, reputation -12..+12 thresholds, hourly mana, favorites lifecycle and potion economy.
- Content tests: authoring matrix coverage, 72 QR slots with Act 1/2/3/Final = 20/22/22/8, 24 always-available/repeatable and 48 story quests, `repeatable_scene`/`always_available_scene`/`unique_object` mix, loc-coded opaque manual IDs as technical slot namespace, PvE tiers 1-4, story-flow fields, 2-of-3 victory rule, reward budgets, act unlock policy, reward approval policy, rarity caps, personal goal hooks, custom Gwent cards, spell/potion minimum catalog, NPC deal flags and final_summary inputs.
